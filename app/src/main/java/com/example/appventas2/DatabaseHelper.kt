...
class DatabaseHelper(context: Context) : SQLiteOpenHelper(context, DATABASE_NAME, null, DATABASE_VERSION) {
    ...
    private fun insertarUsuarioAdmin(db: SQLiteDatabase1) {
        ...
        val pruebas = ContentValues().apply {
            put(COLUMN_USER_CORREO, "pruebas")
            put(COLUMN_USER_PASSWORD, "pruebas")
        }

        db.insert(TABLE_USERS, null, pruebas)

        Log.d("DatabaseHelper", "user registrado añadido.")
